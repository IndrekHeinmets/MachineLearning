...
def get_start_end_barrier_pos(mat):
    start, end, barriers = None, None, []
    for i, row in enumerate(mat):
        for j, node in enumerate(row):
            ...
            if node == '*':
                barriers.append((i, j))
    return start, end, barriers


def main(width):
    map1_mat = map_to_matrix(map1)
    tot_rows = len(map1_mat)

    s, e, b = get_start_end_barrier_pos(map1_mat)
    print(s, '\n', e, '\n', b)
